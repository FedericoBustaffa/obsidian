...
**eso-streams**. Sometimes in fact there is no real stream, like a network
stream or a data stream from a sensor; in that case we can talk about
**endo-stream**.
...
the structure, perform the parallel computation and in the end recollect the
results.
