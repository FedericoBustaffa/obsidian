...
The **map** pattern is a kind of specialization of the _farm_, which involves
the processing of so called **eso-streams**. Sometimes in fact there is no real
stream, like a network stream or a data stream from a sensor; in that case we
can talk about **endo-stream**.

Typically an _eso-stream_ is a stream generated by a loop over a data structure
like an array. This introduce the field of **data parallelism**, in which,
starting from a monolithic data structure, a stream is generated by partitioning
the structure, perform the parallel computation and in the end recollect.
