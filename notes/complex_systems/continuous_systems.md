...
cases the precision of those models is too low. In order to provide more accurate
simulations it's necessary a continuous representation of time.

In other words, the previous time step $\Delta t$ must tend to zero and
introduce a representation through **ordinary differential equations** (ODEs).
We have two fundamental ways to obtain a differential equation from a recurrence
relation:
...
compute the limit for $\Delta t \to 0$ that gives us the **difference quotient**

$$
\lim_{\Delta t \to 0} \frac{N(t + \Delta t) - N(t)}{\Delta t} =
\lim_{\Delta t \to 0} f(N(t))
$$
...
The other case is when we don't have $\Delta t$ in the recurrence and we
_"copy"_ the form of the recurrence as a differential equation. The $N_{t+1}$
part becomes the derivative $\dot{N}(t)$ and the right end side remains the same
but with $N_t$ that becomes $N(t)$.
...
time $t$ let us compute the _change_. In other words the value of the
...
equation. That can be done by find a _closed-form definition_ that depends only
on $t$ and some constants, of $N(t)$ satisfying the equation.

Of course is possibile to find ODEs solutions **analytically**, but there is not
a general method to solve them; in other words we have to reason case by case
and so we cannot automate the process with an algorithm.
...
to compute the **next state**, instead the differential equations describe how
to compute the difference between the current and the next.

Also the **equilibrium** is computed differently; for recurrence equations we
want to know when the next state is equal to the current. In other words we want
to find when the system does not change anymore. In differential equations we
can set the derivative to $0$ so that we reach a local minimum (or maximum) of
the function.
...
value problem is solved by computing a function $F(t)$ that is a solution of the
ODE

$$\dot{N}(t) = f(N(t))$$

and such that $F(0) = N_0$. Usually we are interested in values of $F(t)$ for $t
\geq 0$ since we want to perform a simulation starting from $t = 0$.
...
Let $\dot{N} (t) = f(N(t))$ be the ODE we want to solve and suppose that we know
$N(t_0 = 0)$ ($t_0$ can also be different from $0$) the starting point, and let
$\tau$ be the step size.
...
$N (t_0 + \tau)$ using the tangent instead of the actual function $N$ (that we
...
if we now consider that $m$ is the value of the derivative in $x_0 = t_0$ we can
say that
...
- **Local discretization error**: error raised at every step of the algorithm,
  ...
  $$|N(\tau) - N_{1}|$$
  and is in the order of $O(\tau^2)$.
- **Global discretization error**: the error accumulates and after $k$ steps we
  have an error of
  $$|N(k \tau) - N_{k}|$$
  and it is in the order of $O(\tau)$.

A linear global discretization error means that a very small $\tau$ has to be
used in order to minimize the error; this also can imply a much slower
simulation because smaller $\tau$ means more iterations.

### Implicit Methods

In some cases the method can become unstable unless a very small $\tau$ is used.
This systems are called **stiff systems** and often **implicit methods** are
better for those cases.

An implicit method simply compute the derivative at the end of the time interval
of length $\tau$, so now the formula is slightly different:
