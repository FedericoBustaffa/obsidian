...
In a **discrete dynamical system**, all the variables update at a discrete time-step
and each behavior is modeled by **recurrence relations**. This models are
typically used to model simple _population systems_ where individuals are born,
interact in some way and die.
...
that nothing happens in between $[t, t + \Delta t)$. Another common assumption
is also that if we are modelling something like a population density, a new
individual do not reproduce or die in between the time step he was created and
the next one.
...
problem is possible to get the **general term** (or _solution_) of the
recurrence relation.

This is usually done by trying to understand the form of the _closed formula_ by
...
smaller $\Delta t$ to gain accuracy. Actually we want to let $\Delta t$ tend to
0 and introduce _continuous models_.
