...
Starting for example from a single unit, the model is trained until convergence;
...
![Cascade Correlation|500](/files/cascade_correlation.png)
...
This is done by maximizing a correlation function, that in most cases is the not
normalized covariance between output and error.

$$
S = \sum_k \left| \sum_p
(o_p - \overline{o}) (E_{p,k} - \overline{E}_k) \right|
$$

where

- $k$ and $p$ are respectively the number of output and the number of training
  patterns.
...
- $\overline{E}_k$ is the mean error over all the patterns for output $k$.

In order to optimize the unit we need to maximize the correlation and so is
necessary to compute the derivation of the correlation function:

$$
\frac{\partial S}{\partial w_j} = \sum_k \text{sign}(S_k) \sum_p (E_{p,k} -
\overline{E}_k) \cdot f'(\text{net}_{p}) \cdot I_{p,j}
$$

where $I_{p,j}$ is the input.

The idea to keep in mind is that the algorithm performs two optimizations; the
first wants to minimize the loss, once there is the need to add an hidden unit,
this is trained by maximimizing its correlation with the error.
